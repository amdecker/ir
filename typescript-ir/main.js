...
function changePalette() {
    ...
    for (var x = 0; x < img.width; x++) {
        for (var y = 0; y < img.height; y++) {
            var imgData = ctx2.getImageData(x, y, 1, 1).data.slice(0, 3).toString();
 ...
 * @param image
 */
function identifyPalette(image, cntxt) {
    // convert palette to rgb. originally in YCbCr
    var rgbPalettes = {};
    var numToPalKeys = Object.keys(numToPal).slice();
    for (var k = 0; k < numToPalKeys.length; k++) {
        var pal = numToPal[numToPalKeys[k]].slice();
    ...
    for (var x = 0; x < image.width; x++) {
        for (var y = 0; y < image.height; y++) {
            var toPop = [];
            var rgb = cntxt.getImageData(x, y, 1, 1).data.slice(0, 3);
            for (var k = 0; k < numToPalKeys.length; k++) {
                var pal = rgbPalettes[numToPalKeys[k]];
                var rgbNotInPal = getIndexIn2D(pal, rgb) == -1;
                if (rgbNotInPal) {
            ...
            var foundMatch = numToPalKeys.length == 1;
            if (foundMatch) {
