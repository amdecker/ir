window.onload = () =>{
    var img: any = document.getElementById("ir-img");

    var canvas: any = document.getElementById('can'); //document.createElement("canvas");//
    ...
    {
        var r:number = Math.floor(c[0] + 1.40200 * (c[1] - 128));
        var g:number = Math.floor(c[0] - 0.34414 * (c[2] - 128) - 0.71414 * (c[1] - 128));
        var b:number = Math.floor(c[0] + 1.77200 * (c[2] - 128));
    ...
    {
        var oldPal:number[][] = removeDuplicates(lavaPalette);
        var newPal:number[][] = removeDuplicates(ironPalette);
        for(var i:number = 0; i < oldPal.length; i++)
        {
            oldPal[i] = YCbCrTorgb(oldPal[i]);
        }
        for(var i:number = 0; i < newPal.length; i++)
        {
        ...
        // change the pixels
        for(var x:number = 0; x < img.width; x++)
        {
            for(var y:number = 0; y < img.height; y++)
            {
                var imgData:string = ctx.getImageData(x, y, 1, 1).data.slice(0, 3).toString()
                var newColor:ImageData = ctx.createImageData(1, 1);
                for(var i:number = 0; i < oldToNew[imgData].length; i++)
                {
    ...
    function removeDuplicates(arr:any[])
    {
        ...
        for(var i:number = 0; i < arr.length; i++)
        {
            ...
            for(var n:number = 0; n < noDup.length; n++)
            {
                if(arr[i] == noDup[n])
                {
                    shouldAdd = false;
    ...
    {
        var newArr:any[] = fill(new Array(newLength), null);
        var numEach:number = Math.round(newArr.length / origArr.length);
        var prev:number = 0;
        var i:number = 0;
